...
type: docs
url: /translation/file/sdk/subtitles/
productName: "GroupDocs.Translation Cloud"
title: Translating Srt files with GroupDocs.Translation SDK
description: How to use GroupDocs.Translation Cloud SDK for translating Srt files.
keywords:
- translate
- API
- program
...
Although you can directly call the GroupDocs.Translation Cloud REST API to [send Srt file for translation](/translation/file/request/subtitles/) and [fetch translated file](/translation/file/fetch/), there is a much easier way to implement translation functionality in your applications. We provide software development kits (SDKs) for all popular programming languages. They wrap up all routine operations such as establishing connections, sending API requests, and parsing responses into a few simple methods. It makes interaction with GroupDocs.Translation Cloud services much easier, allowing you to focus on business logic rather than technical details.
...
namespace GroupDocs.Translation.Cloud.Sdk
{
	public class FileTranslator
	{
		public FileTranslator()
		{
			...
			string filePath = "/path/to/myfile.srt";
			...
			var request = new SrtFileRequest(
				sourceLanguage: sourceLanguage,
				targetLanguages: targets,
				url: url,
				savingMode: SrtFileRequest.SavingModeEnum.Files);
			/** Send file to translation */
			var responseId = await api.SrtPostAsync(request);			
...
from groupdocs_translation_cloud import SrtFileRequest, Format
...
url = file_api.file_upload_post(file="/path/to/yourfile.srt")
file_request = SrtFileRequest(source_language="en", 
							           target_languages=["ru"], 
							           url=url, 
							           saving_mode=SavingMode.Files, 
							           output_format=Format.Srt)
response = api.srt_post(file_request)
...
public class StrDemo {
    public static void main(String[] args) {
        ...
        SrtFileRequest fileRequest = new SrtFileRequest();
        ...
        try {
            StatusResponse response = translationApi.srtPost(fileRequest);
